...
function AskChip({ask,placement='right',onOpen}:{ask:AskPrompt;placement?:'left'|'right'|'above';onOpen:()=>void}){
  return <motion.div className={`ask-prompt ask-${placement}`} onPointerDown={event=>event.stopPropagation()} initial={{opacity:0,scale:.72,y:5}} animate={{opacity:1,scale:1,y:0}}>
    <motion.button className="ask-prompt-icon" onClick={event=>{event.stopPropagation();onOpen();}} whileHover={{scale:1.08,y:-1}} whileTap={{scale:.94}} aria-label={`Open ${ask.sender}'s Ask`} aria-haspopup="dialog"><img src="/ask-icon.svg?v=2" alt="" aria-hidden="true"/></motion.button>
  </motion.div>;
}
...
function FloatingFriend({ friend, index, selected, offset, ask, activity, knockNudge, actionsOpen, onHover, onOpen, onAsk, onKnock, onRevealActions, onDismissKnock, onDismissActions, onKick }:{ friend:Friend; index:number; selected:boolean; offset:BubbleOffset; ask:AskPrompt|null; activity:ActivityState; knockNudge:boolean; actionsOpen:boolean; onHover:(index:number|null)=>void; onOpen:()=>void; onAsk:(ask:AskPrompt)=>void; onKnock:(friend:Friend)=>boolean; onRevealActions:(friend:Friend)=>void; onDismissKnock:()=>void; onDismissActions:()=>void; onKick:(friend:Friend)=>void }) {
  ...
  return (
    ...
    >
      <motion.button className="friend-profile" onClick={openProfile} onPointerDown={beginHold} onPointerMove={moveHold} onPointerUp={endHold} onPointerCancel={cancelHold} onPointerLeave={cancelHold} onContextMenu={event=>{if(canReceiveKnock(activity))event.preventDefault();}} aria-label={`Open ${friend.name}'s latest moment${canReceiveKnock(activity)?'; press and hold for actions':''}`} whileHover={{scale:1.055}} whileTap={{scale:.97}}><span className="friend-float"><motion.span className="portrait" layoutId={`avatar-${friend.name}`} animate={!reduceMotion&&knockImpact?{x:[0,3,-2,2,0],rotate:[0,2,-2,1,0]}:{x:0,rotate:0}} transition={{duration:reduceMotion?0:.72,type:'spring',stiffness:260,damping:13}}><img src={friend.image} alt="" /></motion.span><b>{friend.name}</b></span></motion.button>
...
function AskPromptModal({ask,onClose,onRespond}:{ask:AskPrompt;onClose:()=>void;onRespond:()=>void}){
  const respondButton=useRef<HTMLButtonElement>(null);const sender=friends.find(friend=>friend.name===ask.sender);const headingId=`ask-question-${ask.id}`;
  useEffect(()=>{respondButton.current?.focus();},[]);
  return <motion.div className="modal-layer" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><button className="modal-backdrop" onClick={onClose} aria-label="Close Ask"/><motion.section className="ask-dialog" role="dialog" aria-modal="true" aria-labelledby={headingId} onKeyDown={event=>{if(event.key==='Escape'){event.stopPropagation();onClose();}}} initial={{opacity:0,y:24,scale:.94}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:14,scale:.96}} transition={{type:'spring',stiffness:235,damping:23}}><button className="ritual-close" onClick={onClose} aria-label="Close Ask"><X size={17}/></button><div className="ask-dialog-person">{sender&&<img src={sender.image} alt=""/>}<small>{ask.sender.toUpperCase()} ASKS</small></div><h2 id={headingId}>{ask.text}</h2><button ref={respondButton} className="ask-dialog-respond" onClick={onRespond}>Respond <Plus size={18}/></button></motion.section></motion.div>;
}
...
function KnockPanel({items,onSelect,onClose}:{items:Knock[];onSelect:(knock:Knock)=>void;onClose:()=>void}){
  ...
  return <motion.aside id="knock-inbox" className="notification-panel knock-panel" role="dialog" aria-labelledby="knock-inbox-heading" onKeyDown={event=>{if(event.key==='Escape'){event.stopPropagation();onClose();}}} initial={{opacity:0,y:-14,scale:.95}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:-8,scale:.97}} transition={{type:'spring',stiffness:250,damping:22}}>
    <div className="panel-head"><div><small>KNOCK INBOX</small><h2 id="knock-inbox-heading">Who’s checking in</h2></div><button ref={closeButton} onClick={onClose} aria-label="Close Knocks"><X size={17}/></button></div>
...
export default function Home() {
  ...
  return <main className="friend-space" aria-label={backgroundLabel}>
    ...
    <FriendSpace selected={selected} asks={asks} activityOverrides={activityOverrides} nudgeFriend={nudgeFriend} actionFriend={actionFriend} removedFriendNames={removedFriendNames} onOpen={openFriend} onUser={()=>openProfile('you')} onAsk={openAsk} onKnock={sendKnock} onRevealActions={revealActions} onDismissKnock={()=>setNudgeFriend(null)} onDismissActions={()=>setActionFriend(null)} onKick={kickOut}/>
    ...
    <AnimatePresence>{inviteOpen&&<motion.aside id="quick-invite" className={`quick-invite${circleAtCapacity?' is-full':''}`} role="dialog" aria-modal="false" aria-labelledby="quick-invite-title" initial={{opacity:0,y:10,scale:.96}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:6,scale:.97}}><button ref={inviteCloseRef} onClick={()=>setInviteOpen(false)} aria-label="Close invite"><X size={16}/></button><small>INVITE TO YOUR CIRCLE · {circleSeatsUsed}/{MAX_CIRCLE_FRIENDS}</small><h2 id="quick-invite-title">{circleAtCapacity?'Your Circle is full':'Someone missing?'}</h2><p className="invite-capacity">{circleAtCapacity?`Maximum ${MAX_CIRCLE_FRIENDS} friends${pendingInvites.length>0?` · ${pendingInvites.length} invite pending`:''}.`:`${circleSpotsLeft} ${circleSpotsLeft===1?'spot':'spots'} left in your Circle.`}</p><form onSubmit={sendCircleInvite}><input type="email" aria-label="Email address" placeholder={circleAtCapacity?'Maximum reached':'friend@email.com'} value={inviteEmail} onChange={event=>setInviteEmail(event.target.value)} disabled={circleAtCapacity} required/><button type="submit" aria-label="Send invite" disabled={circleAtCapacity||inviteEmail.trim().length===0}><Send size={16}/></button></form></motion.aside>}</AnimatePresence>
