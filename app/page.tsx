...
const initialUnseenPostNames=friends.filter(friend=>prototypeNow-friend.lastPostedAt<=DAY).map(friend=>friend.name);
...
const initialProfilePosts:PhotoPost[]=[
  ...
  {id:'you-coast',authorId:'you',photo:profilePhotoPool[1],caption:'A slow afternoon by the water.',createdAt:prototypeNow-24*DAY,audience:friends.map(friend=>friend.name)},
  {id:'you-old-road',authorId:'you',photo:profilePhotoPool[5],caption:'Still thinking about this road.',createdAt:prototypeNow-52*DAY,audience:friends.map(friend=>friend.name)},
  ...friends.flatMap((friend,friendIndex)=>{const promptText=friend.name==='Vina'?"What's on your desk right now?":friend.name==='Leo'?'What are you snacking on?':undefined;const latestId=`${friend.name}-latest`;return [
    ...
    ...[6,12,24,45].map((daysAgo,postIndex)=>({id:`${friend.name}-archive-${postIndex}`,authorId:friend.name,photo:profilePhotoPool[(friendIndex*2+postIndex+1)%profilePhotoPool.length],caption:profileCaptionPool[(friendIndex+postIndex)%profileCaptionPool.length],createdAt:friend.lastPostedAt-daysAgo*DAY,audience:['You']})),
  ];}),
...
function AskChip({ask,placement='right',now,onOpen}:{ask:AskPrompt;placement?:'left'|'right'|'above';now:number|null;onOpen:()=>void}){
  ...
  return <motion.div className={`ask-prompt ask-${placement}`} onPointerDown={event=>event.stopPropagation()} initial={{opacity:0,scale:.72,y:5}} animate={{opacity:1,scale:1,y:0}}>
    <motion.button className="ask-prompt-icon" onClick={event=>{event.stopPropagation();onOpen();}} whileHover={{scale:1.08,y:-1}} whileTap={{scale:.94}} aria-label={`Open ${ask.sender}'s Ask; ${timeLeft}`} aria-haspopup="dialog" title={timeLeft}><span className="ask-timer-ring" aria-hidden="true"><svg viewBox="0 0 48 48"><circle className="ask-timer-track" cx="24" cy="24" r="20.5"/><motion.circle className="ask-timer-progress" cx="24" cy="24" r="20.5" initial={false} animate={{pathLength:progress}} transition={{duration:.65,ease:[.2,.8,.2,1]}}/></svg></span><img src="/ask-icon.png?v=3" alt="" aria-hidden="true"/></motion.button>
  </motion.div>;
}
...
function FloatingFriend({ friend, index, selected, unseen, offset, orbitCounterRotation, ask, askNow, activity, knockNudge, actionsOpen, onHover, onOpen, onAsk, onKnock, onRevealActions, onDismissKnock, onDismissActions, onKick }:{ friend:Friend; index:number; selected:boolean; unseen:boolean; offset:BubbleOffset; orbitCounterRotation:MotionValue<number>; ask:AskPrompt|null; askNow:number|null; activity:ActivityState; knockNudge:boolean; actionsOpen:boolean; onHover:(index:number|null)=>void; onOpen:()=>void; onAsk:(ask:AskPrompt)=>void; onKnock:(friend:Friend)=>boolean; onRevealActions:(friend:Friend)=>void; onDismissKnock:()=>void; onDismissActions:()=>void; onKick:(friend:Friend)=>void }) {
  const placement:'left'|'right'|'above'=friend.x>25?'right':friend.x<-25?'left':friend.y>100?'above':'left';
  const [knockImpact,setKnockImpact]=useState(false);const [iceBursting,setIceBursting]=useState(false);const [iceDropped,setIceDropped]=useState(false);const [iceBurst,setIceBurst]=useState(0);const reduceMotion=useReducedMotion();const iceLevel=getIceLevel(friend,activity);const temperatureIcon=getTemperatureIcon(iceLevel);
  const impactTimer=useRef<number|undefined>(undefined);const burstTimer=useRef<number|undefined>(undefined);
  const handleImpact=useCallback((active:boolean)=>{if(!active){setKnockImpact(false);setIceBursting(false);return;}if(impactTimer.current)window.clearTimeout(impactTimer.current);if(burstTimer.current)window.clearTimeout(burstTimer.current);setIceDropped(false);setIceBursting(false);setKnockImpact(true);burstTimer.current=window.setTimeout(()=>{setIceBurst(value=>value+1);setIceBursting(true);setIceDropped(true);},300);impactTimer.current=window.setTimeout(()=>{setKnockImpact(false);setIceBursting(false);},1450);},[]);
  ...
  useEffect(()=>()=>{if(holdTimer.current)window.clearTimeout(holdTimer.current);if(impactTimer.current)window.clearTimeout(impactTimer.current);if(burstTimer.current)window.clearTimeout(burstTimer.current);},[]);
  ...
  return (
    <motion.div
      className={`friend activity-${activity} ${activity==='active'||activity==='recentlyActive'?'is-active':'is-inactive'}${selected ? ' is-selected' : ''}${unseen?' has-unseen-post':''}${ask?' has-ask':''}${knockNudge?' has-knock':''}${actionsOpen?' has-profile-actions':''}${iceDropped?' ice-dropped':''}${knockImpact?' is-thawing':''}`}
...
function FriendSpace({ selected, asks, unseenFriendNames, activityOverrides, nudgeFriend, actionFriend, removedFriendNames, canAddFriend, onOpen, onUser, onAddFriend, onAsk, onKnock, onRevealActions, onDismissKnock, onDismissActions, onKick }:{ selected:Friend|null; asks:AskPrompt[]; unseenFriendNames:string[]; activityOverrides:Record<string,ActivityOverride>; nudgeFriend:string|null; actionFriend:string|null; removedFriendNames:string[]; canAddFriend:boolean; onOpen:(friend:Friend)=>void; onUser:()=>void; onAddFriend:()=>void; onAsk:(ask:AskPrompt)=>void; onKnock:(friend:Friend)=>boolean; onRevealActions:(friend:Friend)=>void; onDismissKnock:()=>void; onDismissActions:()=>void; onKick:(friend:Friend)=>void }) {
  ...
  const removedFriendSet=useMemo(()=>new Set(removedFriendNames),[removedFriendNames]);const unseenFriendSet=useMemo(()=>new Set(unseenFriendNames),[unseenFriendNames]);
...
function PostViewer({ friend, post, onClose, onNotify, onViewProfile, onAddYours }:{friend:Friend;post?:PhotoPost;onClose:()=>void;onNotify:(text:string)=>void;onViewProfile:(friend:Friend)=>void;onAddYours:(post:PhotoPost)=>void}) {
  ...
  return <motion.div className="viewer-layer" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} transition={{duration:.2,ease:'easeOut'}}>
    <motion.button className="viewer-backdrop" onClick={onClose} aria-label="Close moment" />
    <motion.article className="post-viewer" initial={{opacity:0,y:10,scale:.975}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:6,scale:.985}} transition={{duration:.24,ease:[.2,.8,.2,1]}}>
      <header className="post-head">
        <div className="post-person"><span style={{background:friend.color}}><img src={friend.image} alt=""/></span><p><b>{friend.name}</b><small>{friend.time}</small></p><button className="post-profile-link" onClick={()=>onViewProfile(friend)} aria-label={`View ${friend.name}'s profile feed`}><UserRound size={14}/><span>Profile</span></button></div>
...
function ProfileFeed({owner,posts,onClose,onCreatePost,onAddYours}:{owner:'you'|Friend;posts:PhotoPost[];onClose:()=>void;onCreatePost:()=>void;onAddYours:(post:PhotoPost)=>void}){
  ...
  return <motion.div className="profile-feed-layer" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
    ...
    <motion.section className="profile-feed" role="dialog" aria-modal="true" aria-labelledby={headingId} onKeyDown={event=>{if(event.key==='Escape'){event.stopPropagation();onClose();}}} initial={{opacity:0,y:34,scale:.97}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:22,scale:.98}} transition={{type:'spring',stiffness:205,damping:24}}>
      <div className="profile-feed-scroll">{posts.length?<><header className="profile-feed-hero"><img className="profile-feed-cover" src={coverPhoto} alt=""/><span className="profile-feed-cover-wash"/><div className="profile-feed-actions">{isSelf&&<button className="profile-feed-create" onClick={onCreatePost}><Plus size={15}/> Post</button>}<button ref={closeButton} className="profile-feed-close" onClick={onClose} aria-label="Back to Circle"><X size={19}/></button></div><span className="profile-feed-avatar" style={{background:person.color}}><img src={person.image} alt=""/></span></header><div className="profile-feed-identity"><h2 id={headingId}>{isSelf?'Your moments':`${person.name}'s moments`}</h2><p>{posts.length} {posts.length===1?'photo':'photos'} shared with the circle</p><small><LockKeyhole size={11}/> Circle only</small></div><div className="profile-feed-timeline">{periods.filter(period=>period.posts.length>0).map(period=><section className={`profile-period profile-period-${period.id}`} aria-labelledby={`${headingId}-${period.id}`} key={period.id}><header><h3 id={`${headingId}-${period.id}`}>{period.label}</h3><span>{period.posts.length}</span></header><div className="profile-feed-grid">{period.posts.map(renderCard)}</div></section>)}</div></>:<div className="profile-feed-empty"><ImagePlus size={26}/><b>No moments yet</b><small>{isSelf?'Your next post will appear here.':`${person.name} hasn’t shared anything yet.`}</small></div>}</div>
    </motion.section>
  </motion.div>;
}
...
export default function Home() {
  ...
  const markPostSeen=(friend:Friend)=>setUnseenFriendNames(current=>current.includes(friend.name)?current.filter(name=>name!==friend.name):current);
  const openFriend=(friend:Friend)=>{markPostSeen(friend);setActiveSurface({kind:'post',friend});setNotificationsOpen(false);setKnocksOpen(false);setActionFriend(null);};
  ...
  const openAsk=(ask:AskPrompt)=>{setNotificationsOpen(false);setKnocksOpen(false);setActionFriend(null);if(ask.sender==='You'){setResponsesAsk(ask);return;}const friend=friends.find(item=>item.name===ask.sender);if(friend){markPostSeen(friend);setActiveSurface({kind:'post',friend,promptId:ask.id});}};
