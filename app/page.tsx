...
function AskChip({ask,placement='right',onOpen}:{ask:AskPrompt;placement?:'left'|'right'|'above';onOpen:()=>void}){
  ...
  return <motion.div ref={root} className={`ask-prompt ask-${placement}${expanded?' is-open':''}`} onPointerDown={event=>event.stopPropagation()} onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setExpanded(false);}} initial={{opacity:0,scale:.72,y:5}} animate={{opacity:1,scale:1,y:0}}>
    <motion.button ref={trigger} className="ask-prompt-icon" onClick={event=>{event.stopPropagation();setExpanded(value=>!value);}} whileHover={{scale:1.08,y:-1}} whileTap={{scale:.94}} aria-label={`Open ${ask.sender}'s Ask`} aria-expanded={expanded} aria-controls={popoverId}><img src="/ask-icon.svg?v=2" alt="" aria-hidden="true"/></motion.button>
...
export default function Home() {
  ...
  const toastTimer=useRef<number|undefined>(undefined);const responseTimers=useRef<number[]>([]);const bellRef=useRef<HTMLButtonElement>(null);const inviteTriggerRef=useRef<HTMLButtonElement>(null);const inviteCloseRef=useRef<HTMLButtonElement>(null);const inviteWasOpen=useRef(false);const incomingKnocks=knocks.filter(knock=>knock.to==='You'&&knock.status==='waiting');const knockComposer=knocks.find(knock=>knock.id===knockComposerId)||null;const knockResponse=knocks.find(knock=>knock.id===knockResponseId&&knock.status==='responded')||null;
  ...
  return <main className="friend-space" aria-label={backgroundLabel}>
    <header className="circle-header"><div><h1>Circle</h1><p>Your people. Closer.</p></div><nav><button ref={bellRef} className="bell" onClick={()=>setNotificationsOpen(v=>!v)} aria-label={notificationsOpen?'Close notifications':'Open notifications'} aria-expanded={notificationsOpen}><Bell size={21} strokeWidth={1.8}/><span/></button></nav></header>
