...
function ProfileActions({friend,onKnock,onKick,onDismiss,onImpact}:{friend:Friend;onKnock:()=>boolean;onKick:()=>void;onDismiss:()=>void;onImpact:(active:boolean)=>void}){
  const [knocking,setKnocking]=useState(false);const timers=useRef<number[]>([]);const reduceMotion=useReducedMotion();
  useEffect(()=>()=>{timers.current.forEach(timer=>window.clearTimeout(timer));onImpact(false);},[onImpact]);
  const knock=()=>{if(!onKnock())return;setKnocking(true);onImpact(true);timers.current.push(window.setTimeout(()=>onImpact(false),720),window.setTimeout(onDismiss,1150));};
  return <motion.div className={`profile-actions${knocking?' is-knocking':''}`} role="group" aria-label={`Actions for ${friend.name}`} onPointerDown={event=>event.stopPropagation()} initial={{opacity:0,scale:.72,rotate:-8}} animate={{opacity:1,scale:1,rotate:0}} exit={{opacity:0,scale:.8,rotate:6}} transition={{type:'spring',stiffness:260,damping:19}}>
    <motion.button className="profile-action profile-action-knock" onClick={knock} disabled={knocking} aria-label={`Knock on ${friend.name}`} title="Knock" animate={!reduceMotion&&knocking?{x:[0,-7,2,-6,0],rotate:[0,-11,5,-8,0]}:{x:0,rotate:0}} transition={{duration:reduceMotion?0:.7,ease:'easeInOut'}}><span aria-hidden="true">👊</span></motion.button>
    <motion.button className="profile-action profile-action-remove" onClick={onKick} disabled={knocking} aria-label={`Remove ${friend.name} from your circle`} title="Remove from Circle" whileHover={{scale:1.08}} whileTap={{scale:.9}}><X size={28} strokeWidth={3.2}/></motion.button>
  </motion.div>;
}

function FloatingFriend({ friend, index, selected, offset, ask, askNow, activity, knockNudge, actionsOpen, onHover, onOpen, onAsk, onKnock, onRevealActions, onDismissKnock, onDismissActions, onKick }:{ friend:Friend; index:number; selected:boolean; offset:BubbleOffset; ask:AskPrompt|null; askNow:number|null; activity:ActivityState; knockNudge:boolean; actionsOpen:boolean; onHover:(index:number|null)=>void; onOpen:()=>void; onAsk:(ask:AskPrompt)=>void; onKnock:(friend:Friend)=>boolean; onRevealActions:(friend:Friend)=>void; onDismissKnock:()=>void; onDismissActions:()=>void; onKick:(friend:Friend)=>void }) {
  ...
  return (
    ...
    >
      <motion.button className="friend-profile" onClick={openProfile} onPointerDown={beginHold} onPointerMove={moveHold} onPointerUp={endHold} onPointerCancel={cancelHold} onPointerLeave={cancelHold} onContextMenu={event=>{if(canReceiveKnock(activity))event.preventDefault();}} aria-label={`Open ${friend.name}'s latest moment${canReceiveKnock(activity)?'; press and hold for actions':''}`} whileHover={{scale:1.055}} whileTap={{scale:.97}}><span className="friend-float"><motion.span className="portrait" layoutId={`avatar-${friend.name}`} animate={!reduceMotion&&knockImpact?{x:[0,9,-4,3,0],scaleX:[1,.86,1.07,.98,1],scaleY:[1,1.08,.94,1.02,1],rotate:[0,4,-3,1,0]}:{x:0,scaleX:1,scaleY:1,rotate:0}} transition={{duration:reduceMotion?0:.62,ease:'easeInOut',times:[0,.16,.42,.7,1]}}>{knockImpact&&<motion.span className="knock-impact-ring" initial={reduceMotion?{opacity:0}:{opacity:.92,scale:.72}} animate={reduceMotion?{opacity:.45}:{opacity:0,scale:1.42}} transition={{duration:reduceMotion ? .18 : .58,ease:'easeOut'}} aria-hidden="true"/>}<img src={friend.image} alt="" /></motion.span><b>{friend.name}</b></span></motion.button>
      ...
      <AnimatePresence>{actionsOpen&&<ProfileActions friend={friend} onKnock={()=>onKnock(friend)} onKick={()=>onKick(friend)} onDismiss={onDismissActions} onImpact={setKnockImpact}/>}</AnimatePresence>
    </motion.div>
  );
}
    ...
    <section ref={spaceRef} className={`social-space orbital-field${selected ? ' is-muted' : ''}${isDragging?' is-dragging':''}`} aria-label="Your close friends" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerEnd} onPointerCancel={pointerEnd} onClickCapture={event=>{if(suppressClick.current){event.preventDefault();event.stopPropagation();}}}>
      <motion.div className="world-layer" style={{x:smoothWorldX,y:smoothWorldY}}>
        <div className="world-stage">
          ...
          <AnimatePresence>{canAddFriend&&<motion.button className="add-person-bubble" style={{'--add-x':`${ADD_FRIEND_BUBBLE.x}px`,'--add-y':`${ADD_FRIEND_BUBBLE.y}px`,'--bubble-size':addBubbleOffset.size,x:addBubbleOffset.x,y:addBubbleOffset.y} as MotionStyle} onPointerDown={event=>event.stopPropagation()} onClick={onAddFriend} initial={{opacity:0,scale:.75}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:.7}} whileHover={{scale:1.07}} whileTap={{scale:.92}} aria-label="Add a person to your Circle"><Plus size={26} strokeWidth={2.1}/></motion.button>}</AnimatePresence>
        </div>
      </motion.div>
...
function AskComposer({circleFriends,onClose,onSend}:{circleFriends:Friend[];onClose:()=>void;onSend:(text:string,recipients:string[],audienceLabel:string)=>void}){
  const [text,setText]=useState('');const [audience,setAudience]=useState<AskAudience>('everyone');const [selectedGroups,setSelectedGroups]=useState<string[]>(['inner']);const [selectedFriends,setSelectedFriends]=useState<string[]>(()=>['Maya','Vina'].filter(name=>circleFriends.some(friend=>friend.name===name)));
  const availableNames=useMemo(()=>new Set(circleFriends.map(friend=>friend.name)),[circleFriends]);
  const recipients=useMemo(()=>{if(audience==='everyone')return circleFriends.map(friend=>friend.name);if(audience==='group')return Array.from(new Set(askGroups.filter(group=>selectedGroups.includes(group.id)).flatMap(group=>group.members))).filter(name=>availableNames.has(name));return selectedFriends.filter(name=>availableNames.has(name));},[audience,selectedGroups,selectedFriends,circleFriends,availableNames]);
  ...
  return <motion.div className="modal-layer" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><button className="modal-backdrop" onClick={onClose}/><motion.section className="ask-composer" initial={{opacity:0,y:28,scale:.96}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:18,scale:.97}}><button className="ritual-close" onClick={onClose} aria-label="Close Ask composer"><X size={17}/></button><h2>Ask your people something.</h2><label className="ask-question"><span>Your prompt</span><textarea autoFocus value={text} onChange={event=>setText(event.target.value)} maxLength={90} placeholder="Desk photo, right now?"/><b>{text.length}/90</b></label><div className="ask-audience"><span><Users size={14}/> Send to</span><div className="ask-audience-tabs">{(['everyone','group','friends'] as AskAudience[]).map(option=><button className={audience===option?'active':''} onClick={()=>setAudience(option)} key={option}>{option==='group'?'Groups':option[0].toUpperCase()+option.slice(1)}</button>)}</div>{audience==='group'&&<div className="ask-group-options">{askGroups.map(group=>{const groupSize=group.members.filter(name=>availableNames.has(name)).length;return <button className={selectedGroups.includes(group.id)?'active':''} onClick={()=>toggleGroup(group.id)} disabled={groupSize===0} key={group.id}><i>{selectedGroups.includes(group.id)&&<Check size={10}/>}</i><span><b>{group.name}</b><small>{groupSize} people</small></span></button>;})}</div>}{audience==='friends'&&<div className="ask-friend-options">{circleFriends.map(friend=><button className={selectedFriends.includes(friend.name)?'active':''} onClick={()=>toggleFriend(friend.name)} key={friend.name}><img src={friend.image} alt=""/><i><Check size={9}/></i><small>{friend.name}</small></button>)}</div>}</div><div className="ask-send-summary"><LockKeyhole size={13}/><span>Only these {recipients.length} people can see and answer this Ask.</span></div><button className="ask-send" disabled={!text.trim()||recipients.length===0} onClick={()=>onSend(text.trim(),recipients,audienceLabel)}><Send size={15}/> Send Ask</button></motion.section></motion.div>;
}

function AskPromptModal({ask,onClose,onRespond}:{ask:AskPrompt;onClose:()=>void;onRespond:()=>void}){
  const respondButton=useRef<HTMLButtonElement>(null);const sender=friends.find(friend=>friend.name===ask.sender);
  useEffect(()=>{respondButton.current?.focus();},[]);
  return <motion.div className="modal-layer" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><button className="modal-backdrop" onClick={onClose} aria-label="Close Ask"/><motion.section className="ask-dialog" role="dialog" aria-modal="true" aria-label={`${ask.sender} asks: ${ask.text}`} onKeyDown={event=>{if(event.key==='Escape'){event.stopPropagation();onClose();}}} initial={{opacity:0,y:24,scale:.94}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:14,scale:.96}} transition={{type:'spring',stiffness:235,damping:23}}><button className="ritual-close" onClick={onClose} aria-label="Close Ask"><X size={17}/></button><div className="ask-dialog-person">{sender&&<img src={sender.image} alt=""/>}</div><h2>{ask.text}</h2><button ref={respondButton} className="ask-dialog-respond" onClick={onRespond}>Respond <Plus size={18}/></button></motion.section></motion.div>;
}
...
function PostComposer({ circleFriends, onClose, onPosted }:{circleFriends:Friend[];onClose:()=>void;onPosted:(draft:{photo:string;caption:string;audience:string[]})=>void}) {
  const [preview,setPreview]=useState<string|null>(null); const [caption,setCaption]=useState('');
  const [selected,setSelected]=useState(()=>circleFriends.slice(0,8).map(friend=>friend.name));
  ...
  return <motion.div className="modal-layer" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
    ...
    <motion.section className="composer" initial={{opacity:0,y:40,scale:.96}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:20,scale:.97}} transition={{type:'spring',stiffness:190,damping:23}}>
      <div className="composer-head"><span><h2>Share with your people</h2></span><button onClick={onClose} aria-label="Close"><X size={19}/></button></div>
...
export default function Home() {
  ...
  const openAsk=(ask:AskPrompt)=>{setNotificationsOpen(false);setKnocksOpen(false);setActionFriend(null);if(ask.sender==='You')setResponsesAsk(ask);else setPromptAsk(ask);};
  ...
  return <main className="friend-space" aria-label={backgroundLabel}>
    ...
    <button className="post-action" onClick={()=>{setActionFriend(null);setComposerOpen(true);}}><span><Plus size={30}/></span><b>Post</b></button>
    ...
    <AnimatePresence>{composerOpen&&<PostComposer circleFriends={circleFriends} onClose={()=>setComposerOpen(false)} onPosted={posted}/>}</AnimatePresence>
    <AnimatePresence>{askComposerOpen&&<AskComposer circleFriends={circleFriends} onClose={()=>setAskComposerOpen(false)} onSend={sendAsk}/>}</AnimatePresence>
